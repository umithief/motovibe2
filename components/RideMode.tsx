...
export const RideMode: React.FC<RideModeProps> = ({ route, onNavigate }) => {
  ...
  useEffect(() => {
      if (mapContainerRef.current && !mapRef.current && typeof L !== 'undefined') {
          // If a route is present, center on it, otherwise default
          const initialCenter = route && route.coordinates 
          ...
          const map = L.map(mapContainerRef.current, { 
              ...
              zoomAnimation: true,
              inertia: true
          }).setView(initialCenter, 17); // Closer zoom for modern nav look
          
          // MODERN DARK MAP (CartoDB Dark Matter) - Always active for "Modern" feel
          ...
          // Initial Marker (will be updated)
          updateMarkerIcon();
          ...
          trailPolylineRef.current = L.polyline([], { 
              ...
              opacity: 0.8,
              lineCap: 'round',
              shadowBlur: 15, // Leaflet doesn't support shadow directly, simulated via CSS on svg path
              className: 'neon-polyline' // We will target this in global CSS
          }).addTo(map);
          
          // Add CSS for the neon line
          const style = document.createElement('style');
          style.innerHTML = `
            .neon-polyline { filter: drop-shadow(0 0 8px #00f3ff); }
  ...
  useEffect(() => {
      if (isDemoMode) {
          // Demo mode overrides GPS
          return;
      }

      // If user turned OFF GPS
      if (!isGpsEnabled) {
      ...
      if (isGpsEnabled && navigator.geolocation) {
          ...
          watchIdRef.current = navigator.geolocation.watchPosition(
              ...
              (err) => {
                  ...
                  setGpsStatus('searching'); // Keep searching or handle error state
              },
              {
  ...
  const updateMarkerIcon = () => {
      ...
      const icon = L.divIcon({ 
          ...
          html: html, 
          iconSize: [48, 48],
          iconAnchor: [24, 24] // Center of the arrow
      });
  ...
  const updatePosition = (lat: number, lng: number, newSpeed: number, newHeading: number) => {
      ...
      // 1. CRITICAL: Always update refs for calculations
      prevSpeedRef.current = newSpeed;

      // 2. MAP UPDATE
      const mapUpdateThreshold = 50; // Very smooth updates
      if (now - lastMapUpdateRef.current > mapUpdateThreshold) {
          
          if (mapRef.current) {
              // Smooth pan to location, keep zoom steady
              mapRef.current.setView([lat, lng], mapRef.current.getZoom(), { animate: true, duration: 0.2 });
          }
          ...
          if (markerRef.current) {
              ...
              if (el) {
                  // Apply rotation to the inner div to avoid rotating the Leaflet container
      ...
      // 3. UI STATE UPDATE
      const uiThreshold = 100;
      if (now - lastUiUpdateRef.current > uiThreshold) {
  ...
  // Routing Effect
  useEffect(() => {
      if (!mapRef.current || typeof L.Routing === 'undefined') return;
      
      // Cleanup previous route explicitly
      if (routingControlRef.current) { 
          try { 
              // Clear waypoints before removing to ensure cleanup
      ...
      // Reset Demo points to prevent loop on old path
      setDemoRoutePoints([]);
      setNextTurn(null);
      setNavMessage(route ? `ROTA: ${route.title}` : 'SİSTEM HAZIR');

      // Only route if we have a current location
      // Since we init currentLoc from route prop, this should be true if a route is selected
      if (!currentLoc) return;

      let waypoints = [];
      if (route && route.coordinates) {
          // Route Start (Current Loc) -> Route End (Defined in Route Data)
          // If route is a path, we could use route.path[0] but route.coordinates is usually the start
          waypoints = [L.latLng(currentLoc.lat, currentLoc.lng), L.latLng(route.coordinates.lat, route.coordinates.lng)];
      } else if (activeTarget) {
          const targetLat = currentLoc.lat + 0.005;
          const targetLng = currentLoc.lng + 0.005;
          waypoints = [L.latLng(currentLoc.lat, currentLoc.lng), L.latLng(targetLat, targetLng)]; 
      }
  ...
  }, [route, activeTarget]); // Dependencies: Trigger when route changes

  // Demo Loop
  useEffect(() => {
      let interval: any;
      if (isDemoMode && demoRoutePoints.length > 0) {
          // Demo overrides GPS status visually
          setGpsStatus('active');
          if (watchIdRef.current) navigator.geolocation.clearWatch(watchIdRef.current);

          const freq = 100; // Smooth 10fps updates for demo
          ...
          // Revert to searching if exiting demo mode and GPS was on
          setGpsStatus('searching');
      } else {
          // Ensure OFF if exiting demo and GPS is off
